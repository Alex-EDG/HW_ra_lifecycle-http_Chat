# Домашнее задание по курсу "React: Библиотека №1 в современной фронтенд-разработке" к занятию «Жизненный цикл и работа с HTTP».
...
- При монтировании компонента запускается интервал или таймаут, который периодически выполняет GET-запрос:  
  ```
  GET http://localhost:7075/messages?from={lastMessageId}
  ```  
...
- Выполняется POST-запрос:  
  ```
  POST http://localhost:7075/messages
